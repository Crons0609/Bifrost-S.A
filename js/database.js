...
function _fallbackWineImage(wine = {}) {
  ...
  return useSecondBottle ? IMG.bottles2 : IMG.bottles1;
}

function _sanitizeWineRecord(wine) {
  if (!wine || typeof wine !== 'object') return wine;

  const normalized = { ...wine };
  const rawImage = typeof normalized.imageUrl === 'string'
    ? normalized.imageUrl.trim().replace(/\\/g, '/')
    : '';

  const looksValid =
    rawImage &&
    !/^(undefined|null)$/i.test(rawImage) &&
    (
      /^(?:https?:)?\/\//i.test(rawImage) ||
      /^(?:data:|blob:|\/)/i.test(rawImage) ||
      /^(\.\.\/)?assets\/images\//i.test(rawImage)
    );

  normalized.imageUrl = looksValid ? rawImage : _fallbackWineImage(normalized);
  return normalized;
}
...
class BifrostDB {
  ...
  _init() {
    return new Promise((resolve, reject) => {
      ...
      const fallbackToLocal = (reason) => {
        if (settled) return;
        if (reason) console.warn('⚠️ Usando almacenamiento local:', reason);
        this._initLocalStorage();
        finish();
      };

      if (!window.indexedDB) {
        fallbackToLocal('IndexedDB no disponible.');
        return;
      }

      initTimeout = setTimeout(() => {
        fallbackToLocal('Tiempo de espera agotado al iniciar IndexedDB.');
      }, DB_INIT_TIMEOUT_MS);
      ...
      request.onerror = () => {
        ...
        fallbackToLocal(request.error?.message || 'Error al abrir IndexedDB.');
      };

      request.onblocked = () => {
        console.warn('IndexedDB quedó bloqueado por otra pestaña o versión anterior.');
        fallbackToLocal('IndexedDB bloqueado.');
      };
      ...
      request.onsuccess = async (event) => {
        try {
          ...
          console.error('Error durante la inicialización de IndexedDB:', error);
          fallbackToLocal(error?.message || 'Inicialización incompleta de IndexedDB.');
        }
      };
    });
  }
  ...
  getAllWines() {
    return new Promise((resolve, reject) => {
      if (!this.useIndexedDB) {
        ...
        resolve(wines.map(_sanitizeWineRecord));
        return;
      }
      const tx = this.db.transaction(STORE_NAME, 'readonly');
      const store = tx.objectStore(STORE_NAME);
      const request = store.getAll();
      request.onsuccess = () => resolve((request.result || []).map(_sanitizeWineRecord));
  ...
  getWineById(id) {
    return new Promise((resolve, reject) => {
      if (!this.useIndexedDB) {
        ...
        const wine = wines.find(w => String(w.id) === String(id)) || null;
        resolve(wine ? _sanitizeWineRecord(wine) : null);
        return;
      }
      const tx = this.db.transaction(STORE_NAME, 'readonly');
      const store = tx.objectStore(STORE_NAME);
      ...
      request.onsuccess = () => {
        if (request.result) {
          resolve(_sanitizeWineRecord(request.result));
        } else {
          // Fallback to Number for legacy local data
          const numId = Number(id);
          if (!isNaN(numId)) {
            const req2 = store.get(numId);
            req2.onsuccess = () => resolve(req2.result ? _sanitizeWineRecord(req2.result) : null);
  ...
  addWine(wine) {
    return new Promise(async (resolve, reject) => {
      wine = _sanitizeWineRecord({ ...wine, id: wine.id || Date.now() });
  ...
  _startWinesSync() {
    _fbReady.then(() => {
      ...
      _fbDb.ref('productos_ecommerce').on('value', (snap) => {
        if (!snap.exists()) return;
        const fbWines = Object.values(snap.val()).map(_sanitizeWineRecord);
